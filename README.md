...
| `fydetabduo` | `aarch64-linux` | Panthor  | FydeTab Duo, uses [fyde-nix](https://github.com/NixOnFyde/fyde-nix) hardware module. |
...
| `Mod + Shift + W`       | Toggle Wayscriber screen annotation overlay ([`wayscriber`](users/programs/misc/wayscriber.nix)) |
...
The configuration handles user scripts in three tiers:

1. **Shared scripts** ([`users/common/scripts/`](users/common/scripts/)): Common scripts used by all users, merged into `~/.local/bin/` using `pkgs.symlinkJoin` in [`users/default.nix`](users/default.nix). Includes `niri-streamer.sh`, `todo.sh`, `schedule.sh`, `task-receiver.sh`, and other workflow tools.
2. **Per-user scripts** ([`users/<user>/scripts/`](users/ami/scripts/)): User-specific scripts, also merged via `pkgs.symlinkJoin`. Ami has `1-4-niri.sh` layout scripts, `anki-pomodoro.sh`, `niri_helpers.sh`, and `startup.sh`. Fynix has only `startup.sh`.
3. **Inlined Nix scripts** ([`userVars.shellScripts`](users/ami/variables.nix)): Declared in each user's `variables.nix` and added to the user profile with `pkgs.writeShellScriptBin` (such as `killclick`, `killcurrent`, `qrscan`, `qrcreate`, and monitor focus helpers).
...
- `rnote` - Sketch & handwritten notes (fynix only).
...
- `wayscriber` - ZoomIt-style live screen annotation daemon (toggled with `Mod + Shift + W`).
...
The `dolphin` module also bundles the [Linux File Converter Addon](https://codeberg.org/Lich-Corals/linux-file-converter-addon), adding a "Convert to..." action to Dolphin's right-click menu. Its Python dependencies are packaged (no pip/venv), automatic self-updates are disabled, and the modern UI is built from source so it works on both x86_64 and aarch64.
